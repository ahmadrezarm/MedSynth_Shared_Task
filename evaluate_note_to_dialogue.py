"""
Shared Task Evaluation Script - Note to Dialogue
This script evaluates participant submissions for note-to-dialogue generation.

Usage:
    python evaluate_note_to_dialogue.py --submission <path_to_submission.csv> --output <output_path.csv>

Submission format:
    CSV file with columns: id, generated_dialogue
    - id: matches the test set IDs
    - generated_dialogue: the model's generated dialogue from clinical note
"""
...
def load_submission(submission_path):
    ...
    try:
        ...
        required_cols = ['id', 'generated_dialogue']
...
def evaluate_submission(submission_df, ground_truth_df):
    ...
    Args:
        submission_df: DataFrame with columns [id, generated_dialogue]
    ...
    predictions = merged['generated_dialogue'].fillna('').astype(str).tolist()
    references = merged['dialogue'].fillna('').astype(str).tolist()
...
def main():
    parser = argparse.ArgumentParser(
        description='Evaluate note-to-dialogue generation submissions',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Example:
    python evaluate_note_to_dialogue.py --submission team_submission.csv --output results.csv
